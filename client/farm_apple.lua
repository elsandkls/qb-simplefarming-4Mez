  ...
  AppleTreeField = {
    [1] = {
        label = "Apple Farm",
        fruit = "Apple",
        db_obj = Config.Inventory['Apple'].db_obj,
        in_game_obj = Config.Inventory['Apple'].in_game_obj,
...
-- 2/24/2025 pure guess this is apple tree zones? come back to later, if zones are missing for the new trees and need to be added. 
...
if Config.UseBlips then
    CreateThread(function()
      for k in pairs(AppleTreeField) do
        local AppleTreeFieldBlip = AddBlipForCoord(AppleTreeField[k].BlipCoord)
          SetBlipSprite(AppleTreeFieldBlip, AppleTreeField[k].Blip)
          SetBlipAsShortRange(AppleTreeFieldBlip, true)
          SetBlipScale(AppleTreeFieldBlip, 0.8)
          SetBlipColour(AppleTreeFieldBlip, AppleTreeField[k].BlipColor)
          BeginTextCommandSetBlipName("STRING")
          AddTextComponentString(AppleTreeField[k].label)
          EndTextCommandSetBlipName(AppleTreeFieldBlip)
  
        local ApplePicking = PolyZone:Create(AppleTreeField[k].zones, {
          name = AppleTreeField[k].label,
          minZ = AppleTreeField[k].minz,
          maxZ = AppleTreeField[k].maxz,
          debugPoly = false
        })
        ...
        ApplePicking:onPlayerInOut(function(isPointInside)
          if isPointInside then
            ...
            for _, v in pairs(AppleTreeZones) do
