...
init_apples() 
init_apple_orchard() 
init_apple_orchard_trees() 
create_blips()
create_zones() 

My_Item_Key  = "Apple"  -- must match Config.My_Item_Key for this item in config_apples.lua
...
local function create_blips()
    ...
    local severity = "NOTICE"
    local message = "Excuting"
    local trace_func = "-base-"
    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)

    if Config.UseBlips then
        CreateThread(function()
            for orchards in pairs(Config.Orchards) do  -- orchards will be Apple_Orchard_1, Apple_Orchard_2, Apple_Orchard_3   
...
local function create_zones()
    local my_script = Config.ScriptName
    local this_func = "create_zones " .. '[' .. My_Item_Key.. ']'
    local severity = "NOTICE"
    local message = "Excuting"
    local trace_func = "-base-"
    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)
        ...
        CreateThread(function()
            for orchards in pairs(Config.Orchards) do  --  orchards will be Apple_Orchard_1, Apple_Orchard_2, Apple_Orchard_3   
                ...
                if ContainsFruit(key_string, key_fruit) then        
                    ...
                    local ApplePicking = PolyZone:Create(
                    ...
                    Config.Orchards[orchards].ActiveZones = ApplePicking

                    ApplePicking:onPlayerInOut(   
                    ...
                    ) -- end ApplePicking:onPlayerInOut 
...
RegisterNetEvent(Config.ScriptName..':'..My_Item_Key..'_'..this_action1, function()
    ...
    local my_script = Config.ScriptName
    local this_func = this_action1..' ClientEvent [' .. My_Item_Key.. ']'
    local severity = "NOTICE"
    local message = "Excuting"
    local trace_func = "-base-"
    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)
...
RegisterNetEvent(Config.ScriptName..':'..My_Item_Key..'_'..this_action2, function()
    ...
    local my_script = Config.ScriptName
    local this_func = this_action2..' ClientEvent [' .. My_Item_Key.. ']'
    local severity = "NOTICE"
    local message = "Excuting"
    local trace_func = "-base-"
    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)
...
RegisterNetEvent(Config.ScriptName..':'..My_Item_Key..'_'..this_action3, function()
    ...
    local my_script = Config.ScriptName
    local this_func = this_action3..' ClientEvent [' .. My_Item_Key.. ']'
    local severity = "NOTICE"
    local message = "Excuting"
    local trace_func = "-base-"
    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)
        ...
        if v.CoolDown then
            ...
            if v.CoolDown == 0 then          
                if Config.Trees[my_key].UniqueTree[Config.tree_count].label == true then        
                    ...
                    severity = "NOTICE"
                    message = "qb-target [" .. v.Name .. " ".. _ .." (".. v.Coords.x .. ",  ".. v.Coords.y.. ",  ".. v.Coords.z .. ") ]"
                    trace_func = my_func
                    PrintConsoleMessage(my_script, my_func, severity, message, trace_func)
